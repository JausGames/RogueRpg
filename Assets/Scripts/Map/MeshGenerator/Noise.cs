...
public static class Noise {
	...
	public static float[,] GenerateNoiseMap(int mapWidth, int mapHeight, BitMap bitmap, Vector2 sampleCentre)
	{
		float[,] noiseMap = new float[mapWidth, mapHeight];
		...
		for (int y = 0; y < mapHeight; y++)
		{
			for (int x = 0; x < mapWidth; x++)
			{
				var bitXRatio = mapWidth / bitmap.bitmap.width;
				var bitYRatio = mapHeight / bitmap.bitmap.height;


				Debug.Log("GenerateNoiseMap, bitXIndex = " + x / bitXRatio + ", bitUIndex = " + y / bitYRatio);

				var pixelcolor = bitmap.bitmap.GetPixel(x/bitXRatio, y/ bitYRatio);
